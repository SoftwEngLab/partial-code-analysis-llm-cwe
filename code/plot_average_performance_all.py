    ...
    for cwe in results.values():
        for exp, metrics in cwe.items():
            if exp in avg_results:
                avg_results[exp]['Overall Accuracy'].extend(metrics['Overall Accuracy'])
                avg_results[exp]['Pairwise Accuracy'].extend(metrics['Pairwise Accuracy'])
                avg_results[exp]['F1 Score'].extend(metrics['F1 Score'])
